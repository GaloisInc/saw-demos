...
from saw_client.llvm import Contract, LLVMType, SetupVal, array_ty, i8, struct

def int_to_8_cryptol(length: int):
    return cry("`{i}:[8]".format(i=length))

def int_to_32_cryptol(length: int):
    return cry("`{i}:[32]".format(i=length))

def int_to_64_cryptol(length: int):
    return cry("`{i}:[64]".format(i=length))
